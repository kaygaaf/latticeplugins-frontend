...
const SLUG = "/blog/woocommerce-slovak-vat-invoices";

export const metadata: Metadata = {
  title: "WooCommerce Slovak VAT Invoices: DPH / IČ DPH Checklist",
  description:
    "A buyer-intent checklist for WooCommerce stores selling in Slovakia: DPH invoice fields, IČ DPH/DIČ/IČO identifiers, EU reverse charge, PDF delivery, credit notes, and accountant export.",
  alternates: {
    canonical: `${SITE_URL}${SLUG}`,
  },
  openGraph: {
    title: "WooCommerce Slovak VAT invoices for EU stores",
    description:
      "Use this Slovak VAT invoice workflow checklist before choosing a WooCommerce invoice plugin for IČ DPH, DIČ, IČO fields, DPH totals, PDF delivery, credit notes, reverse charge, and bookkeeping export.",
    url: `${SITE_URL}${SLUG}`,
...
const slovakInvoiceFields = [
  "Seller legal name, Slovak address, IČ DPH, DIČ, IČO, or EU VAT registration details, invoice identity, bank details, and contact details that match accounting records",
  "Customer legal name, billing address, country, Slovak IČ DPH, DIČ, IČO, or EU VAT number, and optional company, purchase-order, or cost-centre reference",
  "Dedicated invoice number, invoice date, supply date, payment method, currency, net amount, DPH/VAT rate, VAT amount, and gross total",
...
const workflowRows = [
  {
    scenario: "Slovak B2C DPH invoice",
  ...
  {
    scenario: "Slovak B2B order with IČ DPH / DIČ identifiers",
    weakSetup: "Company and IČ DPH/DIČ/IČO values are stored as free-text order notes, so the data is missing from invoice PDFs, email attachments, customer downloads, or accounting exports.",
...
const buyingQuestions = [
  "Can the plugin collect Slovak IČ DPH, DIČ, or IČO identifiers, company names, and EU VAT numbers before checkout is completed?",
  "Can invoice numbers use a dedicated invoice sequence instead of WooCommerce order IDs?",
  "Can the PDF template show seller identity, customer identity, DPH/VAT totals, payment method, VAT identifiers, bank details, and reverse-charge wording?",
...
const faq = [
  {
    q: "Does a Slovak WooCommerce store need more than a payment receipt?",
  ...
  {
    q: "Should Slovak invoice numbers reuse WooCommerce order IDs?",
    a: "A dedicated invoice sequence is safer because WooCommerce order IDs include pending, failed, cancelled, draft, and test orders. A sequence such as SK-2026-000184 is easier to audit and hand to an accountant.",
  },
  {
    q: "What should a Slovak VAT invoice workflow do for EU B2B orders?",
  ...
  {
    q: "How does Lattice Invoices fit this Slovak VAT workflow?",
...
const mailto =
  "mailto:support@latticeplugins.com?subject=Slovak%20WooCommerce%20VAT%20invoice%20workflow%20review&body=Hi%20Lattice%2C%0A%0AI%20want%20a%20Slovak%2FEU%20VAT%20invoice%20workflow%20review%20for%20Lattice%20Invoices%20early%20access.%0A%0AStore%20URL%3A%20%0ACountry%3A%20Slovakia%20%2F%20EU%3A%20%0AB2B%20or%20B2C%3A%20%0ACurrent%20invoice%20plugin%3A%20%0AInvoice%20number%20format%3A%20%0APayment%20methods%3A%20%0ATax%20number%2FVAT%20fields%20needed%3A%20%0AReverse%20charge%20needed%3A%20%0AAccountant%20export%20needed%3A%20";

const articleSchema = {
  "@context": "https://schema.org",
  "@type": "Article",
  headline: "WooCommerce Slovak VAT invoices: DPH and IČ DPH plugin checklist",
  description:
    "A WooCommerce Slovak VAT invoice workflow checklist for EU stores evaluating invoice plugins, IČ DPH, DIČ, IČO fields, DPH totals, PDF delivery, credit notes, reverse charge, and accounting export.",
...
export default function WooCommerceSlovakVatInvoicesPage() {
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="bg-gradient-to-br from-slate-950 via-indigo-950 to-sky-700 text-white">
        <div className="max-w-5xl mx-auto px-6 py-16 lg:py-20">
          <p className="uppercase tracking-[0.3em] text-sm text-sky-200 mb-4">Slovak DPH invoices</p>
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6">
            WooCommerce Slovak VAT invoices need IČ DPH/DIČ/IČO fields, retained PDFs, and correction trails — not just an order receipt.
          </h1>
          <p className="text-xl text-sky-50 leading-relaxed max-w-3xl mb-8">
            If your WooCommerce store sells to Slovak customers or EU B2B buyers, invoice setup has to cover IČ DPH/DIČ/IČO capture, DPH/VAT totals, reverse charge, invoice numbering, credit notes, customer downloads, and accountant export before launch.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <article className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-2xl border shadow-sm p-8">
              <h2 className="text-3xl font-bold mb-4">What a Slovakia-ready WooCommerce invoice plugin must store</h2>
              <p className="text-slate-700 leading-relaxed mb-4">
                Slovak invoice workflows break when checkout cannot capture company and IČ DPH/DIČ/IČO data, invoice numbers are tied to WooCommerce order IDs, invoice data only exists inside a PDF, reverse-charge evidence is missing, refunds overwrite originals, or accountant exports miss invoice document relationships.
              </p>
            ...
            <div className="bg-white rounded-2xl border shadow-sm p-8">
              <h2 className="text-3xl font-bold mb-6">Slovak DPH invoice field checklist</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {slovakInvoiceFields.map((field) => (
            ...
            <div className="bg-white rounded-2xl border shadow-sm p-8 overflow-hidden">
              <h2 className="text-3xl font-bold mb-6">Where Slovak invoice workflows break</h2>
          ...
          <aside className="space-y-6">
            <div className="bg-white rounded-2xl border shadow-sm p-6 sticky top-6">
              ...
              <p className="text-slate-600 mb-5">
                Send your Slovak/EU VAT invoice workflow and get a practical fit check for Lattice Invoices early access.
              </p>
