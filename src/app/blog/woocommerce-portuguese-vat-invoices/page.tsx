...
const SLUG = "/blog/woocommerce-portuguese-vat-invoices";

export const metadata: Metadata = {
  title: "WooCommerce Portuguese VAT Invoices: IVA / NIF Checklist",
  description:
    "A buyer-intent checklist for WooCommerce stores selling in Portugal: IVA invoice fields, NIF business IDs, EU reverse charge, PDF delivery, credit notes, and accounting export.",
  alternates: {
    canonical: `${SITE_URL}${SLUG}`,
  },
  openGraph: {
    title: "WooCommerce Portuguese VAT invoices for EU stores",
    description:
      "Use this Portuguese IVA invoice workflow checklist before choosing a WooCommerce invoice plugin for NIF fields, VAT totals, PDF delivery, credit notes, reverse charge, and bookkeeping export.",
    url: `${SITE_URL}${SLUG}`,
...
const portugueseInvoiceFields = [
  "Seller legal name, Portuguese address, VAT/IVA registration details, invoice identity, and contact details that match bookkeeping records",
  "Customer legal name, billing address, country, NIF business ID or EU VAT number, and optional purchase-order or cost-centre reference",
  "Dedicated invoice number, invoice date, supply date, payment method, currency, net amount, IVA/VAT rate, VAT amount, and gross total",
...
const workflowRows = [
  {
    scenario: "Portuguese B2C IVA invoice",
    weakSetup: "WooCommerce sends only a payment receipt and the finance team manually creates a Portuguese VAT invoice when support receives a request.",
    betterSetup: "Issue a final VAT invoice after payment with a dedicated invoice number, IVA breakdown, retained PDF, customer download access, and exportable invoice metadata.",
  },
  {
    scenario: "Portuguese B2B order with NIF",
    ...
    betterSetup: "Capture company and NIF fields as structured checkout metadata, print them on the invoice PDF, expose them on the order, and export them for bookkeeping.",
  },
  {
...
const buyingQuestions = [
  "Can the plugin collect Portuguese NIF, company names, and EU VAT numbers before checkout is completed?",
  "Can invoice numbers use a dedicated invoice sequence instead of WooCommerce order IDs?",
  "Can the PDF template show seller identity, customer identity, VAT totals, payment method, NIF/VAT identifiers, and reverse-charge wording?",
...
const faq = [
  {
    q: "Does a Portuguese WooCommerce store need more than a payment receipt?",
    a: "Usually yes when the store needs IVA/VAT invoice data, NIF capture, B2B VAT-number evidence, dedicated invoice numbers, reverse-charge wording, retained PDFs, customer downloads, credit notes, or accountant exports.",
  },
  {
    q: "Should Portuguese invoice numbers reuse WooCommerce order IDs?",
    a: "A dedicated invoice sequence is safer because WooCommerce order IDs include pending, failed, cancelled, draft, and test orders. A sequence such as PT-2026-000184 is easier to audit and hand to an accountant.",
  },
  {
    q: "What should a Portuguese IVA invoice workflow do for EU B2B orders?",
  ...
  {
    q: "How does Lattice Invoices fit this Portuguese VAT workflow?",
...
const mailto =
  "mailto:support@latticeplugins.com?subject=Portuguese%20WooCommerce%20VAT%20invoice%20workflow%20review&body=Hi%20Lattice%2C%0A%0AI%20want%20a%20Portuguese%2FEU%20VAT%20invoice%20workflow%20review%20for%20Lattice%20Invoices%20early%20access.%0A%0AStore%20URL%3A%20%0ACountry%3A%20Portugal%20%2F%20EU%3A%20%0AB2B%20or%20B2C%3A%20%0ACurrent%20invoice%20plugin%3A%20%0AInvoice%20number%20format%3A%20%0APayment%20methods%3A%20%0ANIF%2FVAT%20fields%20needed%3A%20%0AReverse%20charge%20needed%3A%20%0AAccountant%20export%20needed%3A%20";

const articleSchema = {
  "@context": "https://schema.org",
  "@type": "Article",
  headline: "WooCommerce Portuguese VAT invoices: IVA / NIF plugin checklist",
  description:
    "A WooCommerce Portuguese VAT invoice workflow checklist for EU stores evaluating invoice plugins, NIF fields, IVA totals, PDF delivery, credit notes, reverse charge, and accounting export.",
...
export default function WooCommercePortugueseVatInvoicesPage() {
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="bg-gradient-to-br from-slate-950 via-blue-950 to-cyan-700 text-white">
        <div className="max-w-5xl mx-auto px-6 py-16 lg:py-20">
          <p className="uppercase tracking-[0.3em] text-sm text-cyan-200 mb-4">Portuguese VAT / IVA invoices</p>
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6">
            WooCommerce Portuguese VAT invoices need NIF fields, IVA totals, and correction trails — not just a receipt.
          </h1>
          <p className="text-xl text-cyan-50 leading-relaxed max-w-3xl mb-8">
            If your WooCommerce store sells to Portuguese customers or EU B2B buyers, invoice setup has to cover NIF capture, IVA totals, reverse charge, invoice numbering, credit notes, customer downloads, and accountant export before launch.
          </p>
          <div className="flex flex-col sm:flex-row gap-4">
            <a href={mailto} className="bg-green-500 text-white px-8 py-4 rounded-xl font-semibold hover:bg-green-400 transition shadow-lg text-center">
              Request Portuguese invoice review
            </a>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <article className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-2xl border shadow-sm p-8">
              <h2 className="text-3xl font-bold mb-4">What a Portugal-ready WooCommerce invoice plugin must store</h2>
              <p className="text-slate-700 leading-relaxed mb-4">
                Portuguese invoice workflows break when checkout cannot capture company and NIF data, invoice numbers are tied to WooCommerce order IDs, invoice data only exists inside a PDF, reverse-charge evidence is missing, refunds overwrite originals, or accountant exports miss invoice document relationships.
              </p>
            ...
            <div className="bg-white rounded-2xl border shadow-sm p-8">
              <h2 className="text-3xl font-bold mb-6">Portuguese IVA invoice field checklist</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {portugueseInvoiceFields.map((field) => (
            ...
            <div className="bg-white rounded-2xl border shadow-sm p-8 overflow-hidden">
              <h2 className="text-3xl font-bold mb-6">Where Portuguese invoice workflows break</h2>
          ...
          <aside className="space-y-6">
            <div className="bg-white rounded-2xl border shadow-sm p-6 sticky top-6">
              ...
              <p className="text-slate-600 mb-5">
                Send your Portuguese/EU VAT invoice workflow and get a practical fit check for Lattice Invoices early access.
              </p>
              ...
              <div className="border-t mt-6 pt-6 space-y-3 text-sm text-slate-600">
                <div>✓ NIF and VAT-number workflow</div>
                <div>✓ IVA invoice field checklist</div>
